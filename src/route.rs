...
pub trait RouteActions {
    ...
    fn greedily_generate_route() -> Route {
        let mut route: Vec<u8> = vec![1];
        while route.len() < ROUTE_LENGTH {
            let last_elem = *route.last().unwrap();
            let mut lowest_cost: Option<u16> = None;
            let mut element: Option<u8> = None;
            for i in 2..=ROUTE_LENGTH {
                if route.contains(&(i as u8)) {
                    continue
                }
                let cost = CITY_DISTANCES[(last_elem - 1) as usize][i-1] as u16;
                if let Some(temp) = lowest_cost {
                    if cost < temp {
                        lowest_cost = Some(cost);
                        element = Some(i as u8);
                    }
                } else {
                    lowest_cost = Some(cost);
                    element = Some(i as u8);
        ...
        route.try_into().expect("failed converting route vector into array")
    }
